...
public class LookupEnvironment implements ProblemReasons, TypeConstants {
    ...
    boolean suppressClassPackageConflict = true; //TODO option

    TypeSystem typeSystem;

    public INameEnvironment nameEnvironment;
    public CompilerOptions globalOptions;
    public Set<InstrumentationOptions> instrumentationOptions = Collections.emptySet();
            ...
            if ((packageBinding = parent.getPackage0(compoundName[i])) == null || packageBinding == TheNotFoundPackage) {
                ...
                if(this.nameEnvironment instanceof INameEnvironmentExtension) {
                    ...
                    if (this.nameEnvironment.findType(compoundName[i], parent.compoundName) != null) {
                        if (!suppressClassPackageConflict) {
                            return null;
                        } else {
                            System.out.println("Suppressed Class-Package name conflict: " +
                                    Arrays.stream(compoundName).
                                            map(String::new).collect(Collectors.joining(".")) + " is a class");
                        }
                    }
